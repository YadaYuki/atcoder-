...
x_dp = [[False for _ in range(int(2e4+1))] for _ in range(len(A_odd)+1)]
y_dp = [[False for _ in range(int(2e4+1))] for _ in range(len(A_even)+1)]
...
for i in range(2,len(A_odd)+1):
  for x_idx in range(int(-1e4),int(1e4+1)):
    if x_idx-A_odd[i-1] >= int(-1e4):  
      x_dp[i][coordinate_to_dpidx(x_idx)] = x_dp[i-1][coordinate_to_dpidx(x_idx-A_odd[i-1])] or x_dp[i][coordinate_to_dpidx(x_idx)]
    if x_idx+A_odd[i-1] <= int(1e4):
      x_dp[i][coordinate_to_dpidx(x_idx)] = x_dp[i-1][coordinate_to_dpidx(x_idx+A_odd[i-1])] or x_dp[i][coordinate_to_dpidx(x_idx)]

# print([[i-1e4 for i, x in enumerate(x_dp[j]) if x == True] for j in range(len(x_dp))])
# y_dp
for i in range(1,len(A_even)+1):
  for y_idx in range(int(-1e4),int(1e4+1)):
    if y_idx-A_even[i-1] >= int(-1e4):  
